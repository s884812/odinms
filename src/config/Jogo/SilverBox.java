...
public class SilverBox extends AbstractMaplePacketHandler {
    ...
    public static String Mensagem = "Voce nao tem a chave do Silver Box para abrir esta caixa!";
    /* Inventario Full */
    public static String Inv_Cheio = "Voce nao tem espaco no inventario suficiente para receber o item. Por favor, limpar algum espaco do seu ";
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (!c.getPlayer().haveItem(5490000)) {
            c.getPlayer().dropMessage(5, Mensagem);
            return;
        }
        ...
        if (itemid == 4280001) {
            ...
            } /* Super Raros */ else if (chance >= 0.06 && chance < 0.35) {
            ...
            if (chance < 0.1) {
                prizeid = rare[new Random().nextInt(rare.length)];
            } /* Super Raros */ else if (chance >= 0.11 && chance < 0.35) {
                prizeid = uncommon[new Random().nextInt(uncommon.length)];
            } /* Comuns */ else {
                prizeid = common[new Random().nextInt(common.length)];
            }
