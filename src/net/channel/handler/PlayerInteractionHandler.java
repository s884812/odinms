        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) { // Trade
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    if (c.getPlayer().haveItem(itemId, 1, false, true)) {
                        ...
                        AutobanManager.getInstance().autoban(c, "LeaderMS | Merchant Shop : Tente abrir uma loja sem o item.");
                        return;
                    }
                } else {
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                if (ivItem != null && ivItem.getQuantity() >= bundles * perBundle) {
                    if (price < 0) {
                        AutobanManager.getInstance().autoban(c, "LeaderMS| PE Adicionar um item com preco negativo.");
                        return;
                    }
            ...
            if (!shop.isOwner(c.getPlayer())) {
                AutobanManager.getInstance().autoban(c, "LeaderMS | Tentar remover item de : " + shop.getOwnerName());
                return;
            }
            if (shop != null && shop.isOwner(c.getPlayer())) {
