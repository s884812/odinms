...
public class UseCashItemHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (player.haveItem(itemId)) {
                ...
                switch (itemType) {
                    ...
                    case 506: // hmm npe when double clicking..
                        ...
                        if (tagType == 0) { // Item tag.
                            ...
                            if (Incubadora.getIncubatedItem(c)) {
                            ...
                            switch (itemId / 1000 % 10) {
                                case 1: // Megaphone
                                    if (player.getLevel() >= 10) {
                                        ...
                                        player.dropMessage("Voce precisa ser acima de level 10 pra usar este item!");
                                    }
                                    break;
                                ...
                                case 6: //item megaphone
                                    ...
                                    if (slea.readByte() == 1) { //item
                                        ...
                                        if (item == null) //hack
                                        {
                                            ...
                                            player.dropMessage("Voce nao pode trocar este item.");
                                    ...
                                    if (c.getChannelServer().getMapFactory().getMap(loc.map).getForcedReturnId() == 999999999) {//This doesn't allow tele to GM map, zakum and etc...
                                        if (!victim.isHidden() && !victim.isGM()) {
                                            if (itemId == 5041000) { //viprock
                                                ...
                                                new ServernoticeMapleClientMessageCallback(1, c).dropMessage("Ou o jogador nao pode ser encontrado ou voce estava tentando se teletransportar para um local ilegal.");
                                                c.getSession().write(MaplePacketCreator.enableActions());
                                            }
                                        } else {
                                            MapleInventoryManipulator.addById(c, itemId, (short) 1, "Teleport Rock erro (nao encontrado)");
                                        ...
                                        MapleInventoryManipulator.addById(c, itemId, (short) 1, "Teleport Rock erro (nao pode Teleportar)");
                                        new ServernoticeMapleClientMessageCallback(1, c).dropMessage("Voce nao pode se teletransportar para este mapa.");
                                        c.getSession().write(MaplePacketCreator.enableActions());
                                    }
                                } else {
                                    MapleInventoryManipulator.addById(c, itemId, (short) 1, "Teleport Rock erro (nao pode Teleportar)");
                                    c.getPlayer().dropMessage("O jogador esta a tentar teleportar a um mapa desabilitado.");
                                    c.getSession().write(MaplePacketCreator.enableActions());
                                }
                            } else {
                                MapleInventoryManipulator.addById(c, itemId, (short) 1, "Teleport Rock erro (nao encontrado)");
            ...
            player.dropMessage(6, "Tentando pacotes de edicao? Ou nao!");
            return;
        }
    }
