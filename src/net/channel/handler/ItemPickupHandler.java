...
import config.Jogo.CashPQ;
import config.Jogo.Items;
...
public class ItemPickupHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.getPlayer().getGMLevel() == 3) {
            c.getSession().write(MaplePacketCreator.serverNotice(5, "GameMasters JR's nao podem pegar itens."));
            return;
        }
        if (ob instanceof MapleMapItem) {
            ...
            synchronized (mapitem) {
                ...
                for (Pair<?, ?> pair : necksonCards) {
                    if (mapitem.getMeso() <= 0) {
                        if (mapitem.getItem().getItemId() == (Integer) pair.getLeft()) {
                            ...
                            c.getSession().write(MaplePacketCreator.serverNotice(5, "[Sistema de Cash] Voce ganhou LeaderNX (+" + necksonGain + ")."));
                ...
                if (mapitem.getMeso() > 0) {
                    ...
                    c.getPlayer().getCheatTracker().pickupComplete();
                    c.getPlayer().getMap().removeMapObject(ob);
                } else if (MapleInventoryManipulator.addFromDrop(c, mapitem.getItem(), "Picked up by " + c.getPlayer().getId(), true)) {
                    c.getPlayer().getMap().broadcastMessage(
                            MaplePacketCreator.removeItemFromMap(mapitem.getObjectId(), 2, c.getPlayer().getId()),
                            mapitem.getPosition());
                    c.getPlayer().getCheatTracker().pickupComplete();
