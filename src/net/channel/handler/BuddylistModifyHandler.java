...
public class BuddylistModifyHandler extends AbstractMaplePacketHandler {
                ...
                try {
                    ...
                    if (charWithId != null) {
                        ...
                        if (channel != -1) {
                            ...
                            buddyAddResult = channelInterface.requestBuddyAdd(addName, c.getSelectedChannel(), player.getId(), player.getName());
                        } else {
                            Connection con = DatabaseConnection.getConnection();
    ...
    private void notifyRemoteChannel(MapleClient c, int remoteChannel, int otherCid, BuddyOperation operation)
        ...
        if (remoteChannel != -1) {
            ...
            channelInterface.buddyChanged(otherCid, player.getId(), player.getName(), c.getSelectedChannel(), operation);
        }
    }
}
