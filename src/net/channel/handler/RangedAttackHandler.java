...
public class RangedAttackHandler extends AbstractDealDamageHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (type == MapleWeaponType.NOT_A_WEAPON) {
            throw new RuntimeException("Jogador " + player.getName() + " esta atacando com algo que nao e uma arma.");
        }
        ...
        if (projectile != 0 || soulArrow || attack.skill == 5121002) {
            ...
            if (mii.isThrowingStar(projectile)) {
                ...
                visProjectile = 0; //bow, crossbow //arrow rain/eruption show no arrows
            }
            ...
            if (skillId != 4001344) { // not lucky 7
                ...
                basedamage = (int) (((player.getTotalLuk() * 5.0) / 100.0) * (totalWatk + projectileWatk)); // l7 has a different formula :>
            }
            if (skillId == 3101005) {
                basedamage *= effect.getX() / 100.0; //arrowbomb
            }
            double critdamagerate = 0.0;
            if (player.getJob().isA(MapleJob.ASSASSIN)) {
                ISkill criticalthrow = SkillFactory.getSkill(4100001);
                if (player.getSkillLevel(criticalthrow) > 0) {
