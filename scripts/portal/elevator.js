...
function enter(pi) {
	if(pi.getPlayer().getMapId() == 222020100) {
		if(pi.getPlayer().getClient().getChannelServer().getEventSM().getEventManager("elevator").getProperty("isDown").equals("true")) {
			...
			pi.playerMessage("O elevador nao esta disponivel para o andar neste momento. Por favor, tente novamente mais tarde.");
			return false;
		}
		...
		pi.playerMessage("Voce encontrou um bug! Por favor, reporte este bug.\r\nForum - leaderms.com.br/forum\r\nID - Elavador-PT");
		return false;
	}
}
