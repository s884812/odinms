...
function enter(pi) {
	...
	var playerStatus = pi.isLeader();
		...
		if (playerStatus == false) {
			pi.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(6, "Apenas o lider do grupo pode prosseguir."));
			return false;
		}else if (mobCount !=0 || reactorCount < 7) {
			// do nothing; send message to player
			pi.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(6, "Matar todos os piratas antes de prosseguir."));
			return false;
		}else {
