...
function enter(pi) {
	...
	if (playerS == false) {
		...
		pi.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(6, "Apenas o lider do grupo pode entrar neste portal."));
		return false;
	}else if (mobCount < 1) {
		...
		pi.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(6, "Por favor, matar todos os monstros antes de prosseguir."));
		return false;
	}
}