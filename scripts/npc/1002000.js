...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status >= 27 && mode == 0) {
			cm.sendNext("Ha muito para ver nesta cidade, tambem. Deixe-me saber se voce quer ir para outro lugar.");
			cm.dispose();
			return;
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("Voce quer ir para alguma outra cidade? Com um pouco de dinheiro envolvido, eu posso fazer isso acontecer. E um pouco caro, mas eu posso executar um desconto especial de 90% para iniciantes.");
		} else if (status == 1) {
			cm.sendSimple("E compreensivel que voce pode ser confundido sobre esse lugar se esta e a sua primeira vez. Se voce tem alguma duvida sobre esse lugar, atire.\r\n#L0##bQue tipo de cidades tem aqui em Ilha Victoria?#l\r\n#L1#Por favor, me leve para outro lugar.#k#l");
		} else if (status == 2) {
			if (selection == 0) {
				cm.sendSimple("Ha sete cidades grandes aqui em Victoria Island. Qual dessas voce quer saber mais?\r\n#b#L0##m104000000##l\r\n#L1##m102000000##l\r\n#L2##m101000000##l\r\n#L3##m100000000##l\r\n#L4##m103000000##l\r\n#L5##m120000000##l\r\n#L6##m105040300##l");
			} else if (selection == 1) {
				status = 26;
				if (cm.getJob().equals(MapleJob.BEGINNER)) {
					var selStr = "Ha um desconto especial de 90% para todos os iniciantes. Tudo bem, onde voce quer ir?#b";
					for (var i = 0; i < maps.length; i++) {
						selStr += "\r\n#L" + i + "##m" + maps[i] + "# (" + costBeginner[i] + " mesos)#l";
					}
				} else {
					var selStr = "Oh, voce nao e um novato, hein? Entao, eu tenho medo que eu possa ter que cobrar o preco alto. Onde voce gostaria de ir?#b";
					for (var i = 0; i < maps.length; i++) {
			...
			cm.sendYesNo("Eu acho que voce nao precisa estar aqui. Voce realmente quer se mudar para #b#m" + maps[selection] + "##k? Bem, vai custar-lhe #b" + sCost + " mesos#k. O que voce acha?");
			selectedMap = selection;
		} else if (status == 28) {
			if (cm.getPlayer().getMeso() < sCost) {
				cm.sendNext("Voce nao tem mesos suficiente. Com suas habilidades, voce deve ter mais do que isso!");
			} else {
				cm.gainMeso(-sCost);
