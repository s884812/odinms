	...
	if (mode == -1) {
		...
		if (status == 0) {
			if (cm.getPlayer().getMarriageQuestLevel() == 0 && cm.getPlayer().getLevel() >= 10) {	
				...
				if (numberOfLoves >= 4) {
					cm.sendNext("Wow, your'e back pretty early. Got the #bProof of Loves#k? Lets see...");
				} else {
			...
			if (cm.getPlayer().getMarriageQuestLevel() == 0 && cm.getPlayer().getLevel() >= 10) {
				...
				if (ringSelection == 0) {
					...
					if (cm.haveItem(4021009, 1) && cm.haveItem(4021007, 1) && cm.getPlayer().getPlayer().getMeso() >= 2000000) {
					...
					if (cm.haveItem(4011006, 1) && cm.haveItem(4021007, 1) && cm.getPlayer().getPlayer().getMeso() >= 1000000) {
