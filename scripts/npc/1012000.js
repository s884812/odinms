...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("Ola! Eu dirijo o taxi regular. Se voce quiser ir de cidade em cidade de maneira segura e rapida, em seguida, entrar em nosso taxi. Teremos muito prazer em leva-lo ao seu destino com um preco acessivel.");
		} else if (status == 1) {
