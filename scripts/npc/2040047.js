...
function action(mode, type, selection) {
    ...
    if (mode == -1) {//ExitChat
        ...
        if(cm.getPlayer().getMap().getId() == 922010000){
            if(status == 0){
                cm.sendNext("Entendo. Junte a forca dos membros do seu grupo e se esforce mais!");
            }else if (status == 1){
                cm.warp(221024500);
                cm.dispose();
            }
            ...
            if (status == 0) {
                cm.sendYesNo("Voce vai precisar comecar do zero se quiser se arriscar nesta missao depois de sair deste estagio. Tem certeza de que deseja sair deste mapa?");
            }else if (status == 1) {
                cm.sendNext("Tudo bem, irei leva-lo para fora!");
            }else if (status == 2) {
