...
function end(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			qm.sendNextPrev("3Rainbow-colored Snail Shell wasn't an item that makes dreams come true. It turns people who use it into snails. Phew... learned a good lesson not to believe in rumors, otherwise I would be the one crawling about.");
		} else if (status == 2) {
			//qm.gainExp(22500);
			//qm.gainMeso(90000);
			//qm.gainFame(3);
